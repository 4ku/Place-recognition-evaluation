...
// finds distance between 2 ids
double SCManager::getDistance(int idx1, int idx2)
{

    // get 2d scan contexts for both ids:
    auto context1 = polarcontexts_[idx1];
    auto context2 = polarcontexts_[idx2];

    // returns dist and shift:
    std::pair<double,int> dist_shift = distanceBtnScanContext(context1, context2);

    return dist_shift.first;
}
